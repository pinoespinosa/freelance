...
def loadFileHoja2():
	try:
		file = open("hoja2.csv", "r") 
		for line in file:
				codigo = line.split(',')[0].strip()
				precio = line.split(',')[1].strip()
				hoja2[codigo] = precio;
		file.close();
	except (FileNotFoundError):
		print('');
...
for valor in hojaGrales:
	...
	if 'FAMILIA' in codigo:
		...
		if 'MARCA' not in codigo:
			...
			hojaGralesProc.append(codigo + ',' + nombre + ',' + marca + ',' + tipo + ',,' + precio  + ',,,,' + web + ',,' + modelo )
