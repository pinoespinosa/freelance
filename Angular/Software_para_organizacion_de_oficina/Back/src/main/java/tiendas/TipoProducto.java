...
public class TipoProducto {
	...
	public TipoProducto(ProductoAbs prod, CategoriaInfo cat) throws SQLException, NumberFormatException, ClassNotFoundException {
		super();
		...
		if(!prod.getSubtitulo().isEmpty())
			...
			this.setSubtitulo(prod.getMarca());
		this.setNombre(prod.getTitulo());
		this.setMarca(prod.getMarca());
		this.setDescipcion(prod.getDescripcion());
		this.setDescription(prod.getDescripcion().substring(0, Math.min(250, prod.getDescripcion().length())));

		this.setImagen(prod.getUrl_imagen_1());
		this.setSlug(createSlug(prod.getTitulo()));

		this.setKeywords(prod.getTitulo() + ',' + prod.getMarca() + ',' + prod.getCategoria_1() + ','
				+ prod.getCategoria_2() + ',' + prod.getCategoria_3());
		this.setKeywords(getKeywords().replaceAll(" ", ","));
	...
	public static String createSlug(String titulo){
		String slug = titulo.replaceAll(" ", "-").toLowerCase().replaceAll("ñ", "n");
		slug = slug.replaceAll("á","a").replaceAll("é","e").replaceAll("í","i").replaceAll("ó","o").replaceAll("ú","u");
		slug = slug.replaceAll("[^a-zA-Z0-9\\-]+","");
		return slug;
	}
	
	public static int getNextCode() throws NumberFormatException, SQLException, ClassNotFoundException{
		if (nextProductCode == -1)
			nextProductCode = Integer.parseInt(BD.executeSqlQueryString(Script.getLastTipoProductoCode(), "codigo"));
	...
	public String getMarcaID() throws SQLException, ClassNotFoundException {
		return getMarcaFromBD(this.getMarca());
	}
