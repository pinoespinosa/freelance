...
import security.MySavedRequestAwareAuthenticationSuccessHandler;
import web.error.CustomAccessDeniedHandler;
...
public class SecurityJavaConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {// @formatter:off
		http.csrf().disable().authorizeRequests().and().exceptionHandling().accessDeniedHandler(accessDeniedHandler)
				...
				.and().authorizeRequests().antMatchers("/api/csrfAttacker*").permitAll().antMatchers("/api/customer/**")
				.permitAll().antMatchers("/api/foos/**").authenticated().antMatchers("/api/async/**").permitAll()
