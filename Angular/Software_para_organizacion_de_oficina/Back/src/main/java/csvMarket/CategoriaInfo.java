...
public class CategoriaInfo {
	...
	public String getCategoriaID() throws ClassNotFoundException {
		return getCategoriaFromBD(getCategoria());
	}
	
	public String getSubCategoriaID() throws ClassNotFoundException {
		return getSubCategoriaFromBD(getSubCategoria());
	}
	
	public String getSubSubCategoriaID() throws ClassNotFoundException {
		return getSubSubCategoriaFromBD(getCategoria());
	}
	
	private static String getCategoriaFromBD(String categoria) throws ClassNotFoundException {
		if (!categoriaID.containsKey(categoria))
			try {
				categoriaID.put(categoria, BD.executeSqlQueryString(
						"SELECT id FROM "+BD.get_BD_NAME()+".categorias as ti WHERE ti.nombre='" + categoria + "';", "id"));
	...
	private static String getSubCategoriaFromBD(String subcategoria) throws ClassNotFoundException {
		if (!subcategoriaID.containsKey(subcategoria))
			try {
				subcategoriaID.put(subcategoria, BD.executeSqlQueryString(
						"SELECT id FROM "+BD.get_BD_NAME()+".subcategorias as ti WHERE ti.nombre='" + subcategoria + "';", "id"));
			} catch (SQLException e) {
				e.printStackTrace();
			}
	...
	private static String getSubSubCategoriaFromBD(String subsubcategoria) throws ClassNotFoundException {
		if (!subsubcategoriaID.containsKey(subsubcategoria))
			try {
				subsubcategoriaID.put(subsubcategoria, BD.executeSqlQueryString(
						"SELECT id FROM "+BD.get_BD_NAME()+".subsubcategorias as ti WHERE ti.nombre='" + subsubcategoria + "';", "id"));
			} catch (SQLException e) {
				e.printStackTrace();
			}
